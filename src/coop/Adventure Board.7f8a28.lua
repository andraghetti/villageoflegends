...
function onLoad()
    ...
    DiscardPileZone = getObjectFromGUID('34bead')
...
function CreateAdventureButtons(reset)
    ...
    for i = 0, 55, 1 do
        ...
        local horizontal_offset = -0.5 + (i % 10) / 10 * 1.5
        self.createButton({
            function_owner = self, click_function = 'MoveAdventureCard'..i, label = tostring(i),
            position = {horizontal_offset, 1.0, vertical_offset}, width = 200, height = 200,
            scale={0.3, 1.0, 0.8}, color = {0.0, 0.0, 0.0}, font_color = {1.0, 1.0, 1.0},
            hover_color = {0.0, 1.0, 0.0}, press_color = {1.0, 1.0, 0.0}, font_size = 120,
    ...
    self.createButton({
        ...
        position = {-0.8, 1, 1},
        width = 400, height = 200, scale={0.3, 1.0, 0.8},
        color = {1.0, 0.0, 0.0}, font_color = {1.0, 1.0, 1.0}, hover_color = {1.0, 0.5, 0.0},
        press_color = {0.8, 0.0, 0.0}, font_size = 120, tooltip = "Reset the adventure deck"
    })
...
function ResetAdventure()
    ...
    for _, object in pairs(DiscardPileZone.getObjects()) do
        if (object.type == 'Card' or object.type == 'Deck'
            ) and object.name ~= 'CardCustom' then
            object.setPosition(MainAdventureDeckZone.getPosition())
            object.setRotation({0, 180, 180})
